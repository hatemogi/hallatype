...
function 정오판단(현재커서위치: boolean, 지문글자: 글자꼴, 쓴글자: 글자꼴): [정오표, 정오표, 정오표] {
    ...
    if (쓴글자 === 글자없음) {
        ...
        if (쓴글자.한글) {
            const 초 = 판단(지문글자.초성, 쓴글자.초성, 현재커서위치 && 쓴글자.초성 === 0 && 쓴글자.중성 > 0);
            const 중 = 판단(지문글자.중성, 쓴글자.중성, 현재커서위치 && 쓴글자.중성 === 0);
            const 종 = 판단(지문글자.종성, 쓴글자.종성, 현재커서위치 &&
                ((쓴글자.종성 === 0 && 지문글자.종성 > 0) || (쓴글자.종성 > 0 && 지문글자.종성 === 0)));
            return [초, 중, 종];
        } else {
