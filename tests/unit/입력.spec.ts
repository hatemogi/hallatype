...
describe('오토마타', () => {
    ...
    it('상태.S20에서', () => {
        ...
        예상결과([S20, false, 'KeyS'],      [위치.유지, 상태.S30, 없음, 한(1, 5, 4)]);
    ...
    it('상태.S21에서', () => {
        ...
        예상결과([S21, false, 'KeyS'],      [위치.유지, 상태.S30, 없음, 한(1, 16, 4)]);
    });

    it('상태.S30에서', () => {
        const S30: 머신상태 = [상태.S30, 없음, 한(1, 1, 4)];
        예상결과([S30, false, 'Backspace'], [위치.유지, 상태.S20, 없음, 한(1, 1, 0)]);
        예상결과([S30, false, 'KeyS'],      [위치.다음, 상태.S10, 한(1, 1, 4), 한(3, 0, 0)]);
    ...
    it('입력머신', () => {
        ...
        expect(머신.입력([false, 'KeyA'])).toEqual([위치.유지, 없음, 한(1, 1, 16)]);
        expect(머신.입력([false, 'KeyE'])).toEqual([위치.다음, 한(1, 1, 16), 한(4, 0, 0)]);
    });
});
